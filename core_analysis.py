...
def plot_bokeh_subplots(core_data, well_data):
    ...
    # (Note: In the original code this plot swaps x and y.)
    p1_extra = figure(
    ...
    # Arrange left column: if p1c exists, place p1 and p1c side by side;
    # otherwise, use p1 only. Then place p1_extra below.
    if p1c is not None:
        left_top = row(p1, p1c)
    else:
        left_top = p1
    ...
    p4.quad(top=hist, bottom=0, left=edges[:-1], right=edges[1:],
            fill_color="red", line_color="black", alpha=0.6, legend_label="Porosity Histogram")
    ...
    if 'CGD' in core_data.columns:
        ...
        p5.quad(top=hist2, bottom=0, left=edges2[:-1], right=edges2[1:],
                fill_color="blue", line_color="black", alpha=0.6, legend_label="Grain Density")
    ...
    # -------------------------------
    # Assemble the final layout using gridplot with a sizing mode.
    final_layout = gridplot([[left_col, p2, right_col]], sizing_mode="stretch_width")

    # Render the final layout in Streamlit.
    st.bokeh_chart(final_layout)
