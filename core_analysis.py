...
file_descriptions = {
    'Well Logging': (
        ...
        "LAS files contain well logs, which can be processed to generate "
        "different analyses, including gamma ray, resistivity, and density logs."
    ),
    'Core Analysis': (
        "Upload CSV file containing core analysis data. The core analysis data "
        "includes physical properties like porosity, permeability, and grain density "
        "for analyzing reservoir characteristics."
    ),
    'Welly Multi Well Projects': (
        "Upload LAS files for multiple wells. The project involves analyzing data from "
        "several wells to compare geological properties and production information across "
        "different locations."
    ),
    'Survey Data': (
        "Upload LAS file with survey data for well path analysis. This data includes well "
        "deviations, inclination, azimuth, and other survey parameters to analyze the well's trajectory."
    ),
    'Decline Curve Analysis': (
    ...
    'Estimated Ultimate Recovery': (
        "Upload an Excel file containing production data for estimating ultimate recovery. The data helps "
        "estimate the total recoverable reserves using different recovery models."
    )
}
